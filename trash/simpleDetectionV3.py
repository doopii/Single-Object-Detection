...
class ObjectDetectorApp:
    def __init__(self, root, image_path=None, max_dim=400):
        self.root = root
        ...
        self.threshold_str = tk.StringVar(value="0.80")
    ...
    def setup_ui(self):
        ...
        ttk.Label(self.root, text="Select Object", font=("Arial", 12, "bold")).grid(row=0, column=0, sticky="w", padx=pad, pady=(pad, 0))
        ttk.Label(self.root, text="Detection Settings", font=("Arial", 12, "bold")).grid(row=0, column=1, sticky="w", padx=pad, pady=(pad, 0))
        ttk.Label(self.root, text="Detection Result", font=("Arial", 12, "bold")).grid(row=2, column=0, sticky="w", padx=pad, pady=(pad, 0), columnspan=2)
        ...
        # Load image button
        load_btn = ttk.Button(ctrl_frame, text="Load Image", command=self.load_image)
        load_btn.grid(row=0, column=0, pady=(0,10), sticky="ew", columnspan=2)

        # Appearance options frame
        app_frame = ttk.LabelFrame(ctrl_frame, text="Appearance Options")
        app_frame.grid(row=1, column=0, sticky="ew", pady=5, columnspan=2)
        ...
        flex_frame.grid(row=2, column=0, sticky="ew", pady=5, columnspan=2)
        ...
        thresh_frame.grid(row=3, column=0, sticky="ew", pady=5, columnspan=2)
        ...
        btn_frame.grid(row=4, column=0, pady=15, sticky="ew", columnspan=2)
        ...
        self.save_btn.grid(row=5, column=0, pady=(5,0), sticky="ew", columnspan=2)

        # Result canvas & info
        self.result_canvas = tk.Canvas(self.root, width=600, height=400,
    ...
    def load_image_from_cv_image(self, img, filename=""):
        self.orig_img = self.resize_image(img, self.max_dim)
...
if __name__ == "__main__":
    root = tk.Tk()
    app = ObjectDetectorApp(root)  # Start with no image loaded
    root.mainloop()
